...
class ReliabilityDiagram(object):
        ...
        **save_args : args
        ...
        for batch_X, batch_matched, batch_uncertainty, bounds in zip(
            ...
            batch_histograms, batch_num_samples, _, _ = self._miscalibration.binning(
                bounds, batch_X, batch_matched, batch_X[:, 0], batch_uncertainty[:, 0]
            )
        ...
        if num_features == 1:
            fig = self.__plot_confidence_histogram(
                X, ece, matched, histograms, num_samples_hist, bin_bounds, title_suffix, fig
            )

        # one additional feature? compute 1D-plot
        elif num_features == 2:
            fig = self.__plot_1d(
                histograms, num_samples_hist, bin_bounds, title_suffix, feature_names, fig
            )
    ...
    def __interpolate_grid(cls, metric_map: np.ndarray) -> np.ndarray:
        ...
        metric_map[nans] = griddata(
            x(~nans), metric_map[~nans], x(nans), method="cubic", fill_value=mean
        )
        return metric_map
    ...
    def __plot_confidence_histogram(
        ...
        if fig is None:
            fig, axes = plt.subplots(1, squeeze=True, figsize=(11, 8))
        ...
        ax.set_xlabel("Confidence", fontsize=14, **csfont)
        ax.set_ylabel("Accuracy", fontsize=14, **csfont)
    ...
    def __plot_1d(
        ...
        for batch_hist, batch_num_samples, bounds in zip(histograms, num_samples_hist, bin_bounds):
            ...
            result = self._miscalibration.process(
                self.metric, *batch_hist, num_samples_hist=batch_num_samples
            )
            bin_median = (bounds[-1][:-1] + bounds[-1][1:]) * 0.5

            # interpolate missing values
            x = np.linspace(0.0, 1.0, 1000)
            miscalibration = interp1d(
                bin_median, result[1], kind="cubic", fill_value="extrapolate"
            )(x)
    ...
    def __plot_2d(
        ...
        def set_axis(ax, map, vmin=None, vmax=None):
            ...
            ax.set_xticks(np.linspace(0.0, len(bin_bounds[0][1]) - 2, 5))
            ax.set_xticklabels(np.linspace(0.0, 1.0, 5))
            ax.set_yticks(np.linspace(0.0, len(bin_bounds[0][2]) - 2, 5))
